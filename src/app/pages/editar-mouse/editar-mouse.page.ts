...
export class ServiceBDService {
  ...
  private listadoMouse = new BehaviorSubject<any[]>([]); // Observable para mouse

  constructor() { }

  // Método para obtener un producto por su ID (mouse en este caso)
  ...
  // Seleccionar todos los productos (incluyendo mouse)
  seleccionarProductos() {
    return this.database.executeSql('SELECT * FROM producto', []).then(res => {
      ...
      this.listadoMouse.next(items as any[]); // Actualizamos la lista de mouse
    }).catch(e => {
  ...
  // Devuelve un observable para los productos/mouse
  fetchMouse() {
