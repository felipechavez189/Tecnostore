...
export class ServiceBDService {
  ...
  private listadoPCArmado = new BehaviorSubject<any[]>([]); // Observable para PC Armado

  constructor() { }

  // Método para obtener un producto por su ID (PC Armado en este caso)
  ...
  // Seleccionar todos los productos (incluyendo PC Armado)
  seleccionarProductos() {
    return this.database.executeSql('SELECT * FROM producto', []).then(res => {
      ...
      this.listadoPCArmado.next(items as any[]); // Actualizamos la lista de PC Armado
    }).catch(e => {
  ...
  // Devuelve un observable para los productos/PC Armado
  fetchPCArmado() {
